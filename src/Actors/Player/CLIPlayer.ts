  ...
  async handle<T extends Event>(event: T): Promise<void> {
    switch (event.type) {
      ...
      case WIN:
        ...
        const lootstack = event.combatEncounter.enemies.reduce((loot, enemy) => {
          enemy.inventory.forEach(sword => loot.push({value: sword, label: sword.name }))
          return loot;
        }, [] as {value: Sword, label: string}[])
        const grabbed = await multiselect<{value: Sword, label: string}[], Sword>({message: "Here are your loots!", options: lootstack})
      ...
      case SELECT_ACTION:
        if (event.turn.actor === this) {
          const options = event.turn.availableActions.map(action => ({label: action.name, value: action, hint: action.description}))
          const selected = await select<{value: Action, label: string}[], Action>({options, message: "What are you going to do?"})
      ...
      case SELECT_TARGETS:
        if (event.turn.actor === this && event.turn.selectedAction instanceof Attack) {
          ...
          while (selected.length < choices) {
            const selection = await select<{value: Actor, label: string}[], Actor>({options, message: "Pick an enemy to smite"})
            if (typeof selection === "symbol") {
              throw new Error(String(selection))
            }
            options.splice(options.findIndex(o => o.value === selection), 1)
